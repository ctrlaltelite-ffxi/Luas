...
function user_setup()
  ...
  mainList = {'Crocea Mors','Naegling'}
  shieldList  = {'Ammurapi Shield'}
  subList = {"Gleti's Knife",'Daybreak'}
  setList = {'FullDD','HybridDD','PDT','MDT','Hybrid','Idle','Refresh'}
  ...
  skill_spells = {'Temper', 'Temper II', 'Enfire', 'Enfire II', 'Enblizzard', 'Enblizzard II', 'Enaero', 'Enaero II','Enstone', 'Enstone II', 'Enthunder', 'Enthunder II', 'Enwater', 'Enwater II'}
  enfeebling_magic_acc = {'Bind', 'Break', 'Dispel', 'Distract', 'Distract II', 'Frazzle','Frazzle II','Gravity', 'Gravity II','Silence'}
  enfeebling_magic_skill = {'Distract III', 'Frazzle III', 'Poison II'}
  enfeebling_magic_effect = {'Dia', 'Dia II', 'Dia III', 'Diaga', 'Blind', 'Blind II'}
  enfeebling_magic_sleep = {'Sleep', 'Sleep II', 'Sleepga'}
  ...
  send_command('bind f12 gs c lockWeapon')
...
function equip_aftercast()
  if state.DW then
    if mainList[state.Main] == "Crocea Mors" then
      equip({main={ name="Crocea Mors", augments={'Path: C',}}, sub=subList[state.Sub]})
    else
      equip({main=mainList[state.Main], sub=subList[state.Sub]})
    end
  else
    if mainList[state.Main] == "Crocea Mors" then
      equip({main={ name="Crocea Mors", augments={'Path: C',}}, sub=shieldList[state.Shield]})
    else
      equip({main=mainList[state.Main], sub=shieldList[state.Shield]})
    end
  end
...
function self_command(command)
  ...
  elseif command == 'lockWeapon' then 
    if state.WeaponLock then state.WeaponLock = false else state.WeaponLock = true end
    update_displaybox()
  else
  end
end
...
function update_displaybox()
  ...
  displayBox:clear()
  if state.DW then
    displayBox:append("M/S: "..color.Info..mainList[state.Main]..color.Label..' / '..color.Info..subList[state.Sub]..'  ')
  else
    displayBox:append("M/S: "..color.Info..mainList[state.Main]..color.Label..' / '..color.Info..shieldList[state.Shield]..'  ')
  end
  
  displayBox:append(color.Label.."Set: "..color.Info..setList[state.Set]..'  ')
  displayBox:append(color.Label.."Acc: "..color.Info..(state.Acc and 'ON' or 'OFF')..'  ')
  displayBox:append(color.Label.."Weapon Lock: "..color.Info..(state.WeaponLock and 'ON' or 'OFF')..'  ')
